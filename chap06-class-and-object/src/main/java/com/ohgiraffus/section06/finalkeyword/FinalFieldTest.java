package com.ohgiraffus.section06.finalkeyword;
...
public class FinalFieldTest {
    ...
    * 4. 클래스(static) 변수 : 프로그램 start이후 값 변경 불가능
    * 5. non-ststic 메소드 : 메소드 재작성(overriding)불가
...
//    private final int NON_STATIC_NUM;
    ...
    /*
    * 생성자를 이용한 초기화는 불가능하다.
    * 생성자는 인스턴스가 생성되는 시점에 호출이 되기 때문에 그 전에는 초기화가 일어나지 못했다.
    * 하지만 static은 프로그램이 start될 때 할당 되기 떄문에 초기화가 되지 않은 상태로 선언된 것과 동일하여
    * 기본값으로 초기화 된 후 값을 변경하지 못하기 때문에 에러가 발생한다.
    * */

    private static final double STATIC_DOUBLE;
//     *스태틱 이란 메모리를 올려놓았는데 클래스의 변수는 0.0으로 고정되어서 아래식이 의미가 없어 에러가난다
//    public FinalFieldTest(double ststicDouble){
//        STATIC_DOUBLE = ststicDouble;
//    }
    /*
    * 초기화 블럭
    *   값을 지정해주겠다고 초기화해주는 블럭
    * */
    static {
        STATIC_DOUBLE = 0.5;
    }
